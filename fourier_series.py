...
class FourierScene(Scene):
    ...
    def construct(self):
        ...
        # Mobject with point
        square = Tex('S').set_height(4).family_members_with_points()[0]
        ...
        dot = Dot(group[-1].get_points()[-1], color=RED).scale(0.005)
        ...
        path1 = Line(dot.get_center(), dot.get_center() + UP * 0.00001, color=YELLOW).set_stroke(width=1.5)
        self.add(path1)
